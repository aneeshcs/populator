...
N.\ Pacific PC1 (PDO-like) & 0.27 & --- & Sign resolved; residual drift still broad (emu var.\ frac.\ 0.82 vs POP 0.28) \\
\bottomrule
\end{tabular}
\end{table}
...
The PDO PC1 correlation recovered to $+0.27$ (from $-0.21$ at step~52k), resolving
the sign-flip that appeared when the rl=12 checkpoint was evaluated. The emulator's
EOF1 accounts for 82\% of the regional variance (vs.\ POP's 28\%), down from 90\%
at step~52k. The drift is measurably reducing with longer rollout training, but
remains the primary open challenge: a slowly growing mean-state error continues to
project onto the leading North Pacific EOF and will require additional training or
an anomaly-relative framing to fully suppress.
...
emulator's PC1 is now positively correlated with POP ($r=0.27$), recovering
from the sign-flip seen at step~52k. The emulator's EOF1 accounts for 82\% of
the regional variance vs.\ POP's 28\%---down from 90\% at step~52k, indicating
that the residual drift projecting onto the leading North Pacific mode is
slowly reducing as rollout training continues. Further reducing this drift
is the primary remaining challenge for multi-decadal stability.
...
EOF for POP (black) and the emulator (red) over the 25.6-year rollout ($r=0.27$).
The emulator EOF1 accounts for 82\% of regional variance vs.\ POP's 28\%;
residual drift still projects onto the leading mode but is reducing with
longer rollout training.}
\label{fig:pdopc}
\end{figure}
...
\item \textbf{Physically meaningful variability is reproduced with high fidelity.}
  ...
  matched), and the leading North Pacific mode ($r=0.27$, sign recovered) over a
  ...
  without being told explicitly what ENSO or the PDO are. The Ni\~no~3.4
  correlation of 0.96 over a 25.6-year window is a particularly strong result.
...
Pacific EOF (emulator variance fraction 0.82 vs.\ POP's 0.28, improving from
0.90 at rl=12). The stable rollout horizon plateaued at 307 months across both
rl=12 and rl=16 training, suggesting that the instability at month 307 is driven
by a slowly growing mean-state drift rather than by a dynamical instability that
longer rollout training can suppress. Different interventions are needed to break
