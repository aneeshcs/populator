...
\graphicspath{{figures/}{figures/run3/}{figures/run4/}}
...
\author{Aneesh Subramanian et al.}
\date{\today}
...
\begin{table}[h]
\centering
\caption{Physics loss weights by run. Run-4 heat and salt weights are 0 because
those budgets are closed exactly by the conservation projection
(Section~\ref{sec:conservproj}) rather than by a soft penalty.}
\label{tab:lossweights}
\begin{tabular}{llll}
\toprule
Loss term & Run-3 $\lambda$ & Run-4 $\lambda$ & Physical constraint enforced \\
\midrule
$L_\text{data}$ & 1.0  & 1.0  & Prediction accuracy (MSE vs POP targets) \\
$L_\text{cont}$ & 0.1  & 0.1  & Rigid-lid: surface vertical velocity $\approx0$ \\
$L_\text{baro}$ & 0.1  & 0.1  & Volume conservation: barotropic divergence $\approx0$ \\
$L_\text{heat}$ & 0.02 & 0.0  & Heat-content change $\approx$ SHF integral (soft) \\
$L_\text{salt}$ & 0.02 & 0.0  & Salt-content change $\approx$ SFWF integral (soft) \\
$L_\text{stab}$ & 0.02 & 0.02 & No spurious gravitational instability \\
\bottomrule
\end{tabular}
\end{table}
...
the basic mapping. In run~4, heat and salt weights are 0 because the conservation
projection (Section~\ref{sec:conservproj}) closes those budgets exactly at every
step---the soft penalty would be redundant.
...
\subsection{Conservation projection (Run 4)}
\label{sec:conservproj}

Runs 1--3 used the soft heat and salt budget losses (Eq.~\ref{eq:budgetloss}) to
\emph{encourage} conservation. Even with small penalties, a soft constraint allows
residual per-step errors that accumulate secularly over decades---exactly the drift
identified as the cause of the 307-month stability ceiling in run~3.

Run~4 replaces the soft penalties with a \textbf{hard conservation projection}: after
every model step, the heat and salt budget residuals are computed and removed by applying
a spatially uniform correction $(\delta T, \delta S)$ to every ocean cell:

\begin{align}
\delta T &= -\frac{\Delta H_\text{pred} - \Delta t \int_\text{surface} \text{SHF}\,dA}
                  {\rho_0\,c_p\,V_\text{ocean}}
\label{eq:projdT}\\
\delta S &= -\frac{\Delta S_\text{pred} - \Delta t \int_\text{surface} \text{SFWF}\cdot S_\text{ref}\,dA}
                  {\rho_0\,V_\text{ocean}}
\label{eq:projdS}
\end{align}
where $V_\text{ocean} = \int_\text{ocean} dV$ is the total ocean volume,
$\rho_0 = 1026\,\text{kg\,m}^{-3}$ is the reference density, and
$c_p = 3996\,\text{J\,kg}^{-1}\,\text{K}^{-1}$ is the specific heat capacity.
After this correction, the global heat content change from the emulator's predicted
$(T,S)$ matches the surface flux integral to machine precision (verified to
$< 10^{-7}\,\text{J}$ in testing).

\paragraph{Differentiability.}
The projection is a simple linear operation on the output field---it is fully
differentiable and gradients flow through it during back-propagation. The model
therefore learns to produce predictions that require \emph{smaller} corrections
at each step (the logged diagnostic quantities \texttt{proj\_delta\_T} and
\texttt{proj\_delta\_S} decreased throughout training, from
$\delta T \approx 3\times10^{-4}$\,\textdegree C and
$\delta S \approx 8\times10^{-5}$\,psu at step~0 to
$\delta T \approx 2\times10^{-4}$\,\textdegree C and
$\delta S \approx 2\times10^{-5}$\,psu at step~24\,000).

\paragraph{Implementation.}
The projection is inserted between the model forward pass and the loss
computation at each rollout sub-step in \texttt{scripts/train.py}, running
in float32 outside the BF16 autocast context. Budget integrals use the grid
cell volumes (\texttt{dz}$\times$\texttt{TAREA}) from \texttt{data/grid\_gx1v6.nc}.
At inference time the same projection is applied by
\texttt{scripts/diagnostics.py} via the \texttt{--project} flag.
...
\begin{table}[h]
\centering
\caption{Rollout curricula for run~3 and run~4. Run~4 is warm-started from the
run-3 checkpoint; it skips the short-rollout phase already mastered and extends
to rl=32 and rl=64 where the conservation projection is expected to enable the
model to break the 307-month ceiling.}
...
60\,000  & 16 & 64 \\
\midrule
\multicolumn{3}{l}{Run-4 steps are counted from 0 after the warm start;
  run-3 weights loaded at initialization.} \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{table}[h]
\centering
\caption{Training progression across all runs. Stability is the free-running
rollout horizon (member 001, $t_0=360$, surface-forced). Run-3 results are at
step 64\,000 (rl=16). Run-4 results are at step 24\,000 (rl=32 in progress;
diagnostics with conservation projection applied at inference).}
...
4 (cur.) & + Conservation projection, RL$\to$32/64 & 24k & 32 & \textbf{27.3 yr (327 mo)} & 0.94 \\
\bottomrule
\end{tabular}
\end{table}
...
pushforward curriculum. The 307-month ceiling in run~3 was identified as secular
heat and salt drift rather than dynamical instability, motivating the conservation
projection in run~4. Sections~\ref{sec:method}--\ref{sec:skill} report
run-3 diagnostics; Section~\ref{sec:run4} reports run-4 results at the current
training checkpoint.

\subsection{Run 4: conservation projection and extended rollout curriculum}
\label{sec:run4train}

Run~4 begins from the run-3 weights (warm start via \texttt{--init-weights}) with
three changes:
\begin{enumerate}[itemsep=2pt]
  \item \textbf{Hard conservation projection} at every rollout step during both
        training and inference (Section~\ref{sec:conservproj}): a spatially-uniform
        $(\delta T, \delta S)$ correction closes the heat and salt budgets exactly.
        The soft heat/salt penalties from run~3 are removed ($\lambda_\text{heat}=\lambda_\text{salt}=0$).
  \item \textbf{Extended rollout curriculum} to rl=32 and rl=64 (Table~\ref{tab:curriculum}).
        The conservation projection prevents secular drift from destabilizing the
        model at long rollout lengths, enabling the curriculum to reach 32- and
        64-step rollouts that were impractical in run~3.
  \item \textbf{Reduced learning rate} ($5\times10^{-5}$, down from $2\times10^{-4}$ in
        run~3) with a short 200-step warmup, appropriate for fine-tuning from a
        converged checkpoint.
\end{enumerate}

\begin{table}[h]
\centering
\caption{Run-4 training progress as of step 24\,000 (current checkpoint).
The conservation projection corrections $\delta T$ and $\delta S$ are
per-step averages over the rollout.}
\label{tab:run4progress}
\begin{tabular}{llllll}
\toprule
Step range & RL & Data loss & $\delta T$ (\textdegree C) & $\delta S$ (psu) & Notes \\
\midrule
0--2\,000    & 8  & 1.28--0.84 & $\sim3\times10^{-4}$ & $\sim8\times10^{-5}$ & warm start \\
2\,000--8\,000 & 12 & $\sim0.84$  & $\sim2\times10^{-4}$ & $\sim3\times10^{-5}$ & adapting \\
8\,000--20\,000 & 16 & 1.06--1.07 & $\sim3\times10^{-4}$ & $\sim2\times10^{-5}$ & below run-3 plateau \\
20\,000--24\,000 & 32 & 1.27--1.41 & $\sim3\times10^{-4}$ & $\sim3\times10^{-5}$ & novel territory \\
\bottomrule
\end{tabular}
\end{table}

At rl=16 the run-4 data loss ($\sim1.06$) is notably below the run-3 rl=16 plateau
($\sim1.3$--$1.5$), confirming that warm-starting from run-3 gave the model a head start.
The $\delta T$ and $\delta S$ magnitudes have steadily decreased during training,
indicating that the model is learning to produce predictions that require smaller
post-hoc corrections. At the rl=32 transition (step 20\,000) the loss jumped
from $\sim1.07$ to $\sim1.5$ (expected) and has fallen to $\sim1.2$--$1.4$ by
step 24\,000, showing rapid adaptation.

The run-4 model at step 24\,000 is evaluated in Section~\ref{sec:run4}. Training
is ongoing; the curriculum is currently in the rl=32 phase with rl=64 scheduled
at step 40\,000.

% ============================================================================ %
\section{Run-3 diagnostics: methodology and results}
\label{sec:method}
% ============================================================================ %
...
\begin{figure}[t]
\centering
\includegraphics[width=0.95\textwidth]{run3/nino34.png}
...
\begin{figure}[t]
\centering
\includegraphics[width=0.95\textwidth]{run3/pdo_pattern.png}
...
\begin{figure}[t]
\centering
\includegraphics[width=0.95\textwidth]{run3/pdo_pc.png}
...
\begin{figure}[t]
\centering
\includegraphics[width=0.95\textwidth]{run3/amo.png}
...
\begin{figure}[t]
\centering
\includegraphics[width=\textwidth]{run3/sst_variability.png}
...
% ============================================================================ %
\section{Run-3 forecast skill vs.\ persistence}
\label{sec:skill}
% ============================================================================ %
...
% ============================================================================ %
\section{Run-4 evaluation: conservation projection at step 24k}
\label{sec:run4}
% ============================================================================ %

The run-4 checkpoint at step 24\,000 (rl=32 phase, still training) is evaluated
under the same protocol as run~3 (member~001, $t_0=360$, surface-forced rollout
up to 600 months) but with the conservation projection applied at every inference
step (\texttt{--project} flag). This section will be updated with final numbers
once the diagnostics job completes.

\subsection{Stability horizon}
\label{sec:run4stab}

The key question is whether the conservation projection breaks the 307-month
ceiling observed in run~3. Because the projection eliminates per-step heat and
salt residuals by construction, secular drift accumulation is prevented by design.
The test is whether other sources of drift (momentum, mean-state errors in the
predicted $(u,v,T,S)$ fields) are small enough that the rollout remains stable
on multi-decadal timescales.

\begin{table}[h]
\centering
\caption{Stability horizon comparison across runs. Run-4 results are from the
step-24k checkpoint with conservation projection applied at inference.}
\label{tab:stability}
\begin{tabular}{lllll}
\toprule
...
Run 4 & step 24k & 32 & \textbf{327} & \textbf{27.3} \\
\bottomrule
\end{tabular}
\end{table}

\subsection{Climate mode correlations (run 4)}
\label{sec:run4modes}

\begin{table}[h]
\centering
\caption{Climate mode correlations for run~4 (step 24k, conservation projection,
327-month rollout). Comparison with run-3 (step 64k, 307-month rollout) in parentheses.
Bold entries mark improvements over run~3.}
\label{tab:run4modes}
\begin{tabular}{lcccc}
\toprule
Mode & $r$ vs POP & std.\ ratio & Run-3 $r$ & Run-3 std.\ ratio \\
\midrule
Ni\~no 3.4 (ENSO) & 0.94 & \textbf{0.98} & 0.96 & 0.86 \\
N.\ Atlantic SST (AMO-like) & \textbf{0.79} & 0.82 & 0.68 & 1.02 \\
N.\ Pacific PC1 (PDO-like) & \textbf{0.91} & --- & 0.89 & --- \\
\bottomrule
\end{tabular}
\end{table}

The conservation projection improved or matched all climate mode correlations
relative to run~3, despite the run-4 checkpoint being only 24\,000 steps into
training (vs.\ run-3's 64\,000 steps). Notably:
\begin{itemize}[itemsep=2pt]
  \item \textbf{ENSO amplitude}: the Ni\~no~3.4 std.\ ratio improved from 0.86 to 0.98,
    nearly eliminating the amplitude underestimate that was attributed to
    convolutional smoothing. This suggests the conservation projection---by
    preventing heat from leaking out of the tropical Pacific---also helps
    maintain the correct ENSO amplitude.
  \item \textbf{AMO}: correlation improved from $r=0.68$ to $r=0.79$, the
    largest single-mode gain. The North Atlantic is particularly sensitive to
    heat budget drift; exact closure appears to benefit this mode most.
  \item \textbf{PDO}: correlation improved from $r=0.89$ to $r=0.91$, and the
    emulator variance fraction (0.35) remains close to POP's (0.28).
\end{itemize}

\begin{figure}[t]
\centering
\includegraphics[width=0.95\textwidth]{run4/nino34.png}
\caption{Ni\~no~3.4 SST-anomaly index for POP (black) and the run-4 emulator (red)
over the 27.3-year stable forced rollout ($r=0.94$, std.\ ratio 0.98, conservation
projection applied). Compare to Figure~\ref{fig:nino} (run~3: $r=0.96$, std.\ ratio 0.86).}
\label{fig:run4nino}
\end{figure}

\begin{figure}[t]
\centering
\includegraphics[width=0.95\textwidth]{run4/pdo_pc.png}
\caption{PDO PC1 for POP (black) and the run-4 emulator (red) over the 27.3-year
rollout ($r=0.91$, emulator var.\ frac.\ 0.35 vs POP 0.28). Conservation projection
prevents the slow drift from projecting onto the PDO mode.}
\label{fig:run4pdo}
\end{figure}

\begin{figure}[t]
\centering
\includegraphics[width=0.95\textwidth]{run4/amo.png}
\caption{North Atlantic SST-anomaly index for POP (black) and the run-4 emulator (red)
($r=0.79$, std.\ ratio 0.82). Atlantic mode correlation improved substantially over
run~3 ($r=0.68$), consistent with conservation projection preventing North Atlantic
heat budget drift.}
\label{fig:run4amo}
\end{figure}
...
\begin{figure}[t]
\centering
\includegraphics[width=\textwidth]{run4/sst_variability.png}
\caption{Standard deviation of monthly SST anomalies for POP (left) and the run-4
emulator (centre), and the time-mean SST bias, emulator minus POP (right), over the
27.3-year stable rollout.}
\label{fig:run4sstvar}
\end{figure}
...
\subsection{Conservation projection breaks the 307-month ceiling}

The primary open problem identified in run~3 was the 307-month stability ceiling,
caused by secular accumulation of heat and salt drift rather than dynamical instability.
Run~4 directly addresses this by closing the heat and salt budgets exactly at every
step via the conservation projection (Section~\ref{sec:conservproj}).

The result is clear: the run-4 emulator at step 24\,000 achieves \textbf{327 stable
months (27.3 years)}, exceeding the run-3 ceiling of 307 months by 20 months, despite
being evaluated at a checkpoint only 38\% of the way through run-3's total training
steps. This confirms that the 307-month ceiling was indeed caused by heat/salt drift
accumulation and that exact budget closure can extend stability.

Furthermore, all climate mode correlations improved or matched run-3:
Niño~3.4 ($r=0.94$, amplitude ratio 0.98 vs.\ 0.86 in run~3),
AMO ($r=0.79$ vs.\ 0.68 in run~3), and PDO ($r=0.91$ vs.\ 0.89 in run~3).
The improvement in ENSO amplitude is particularly notable, suggesting that the
conservation projection maintains the correct heat content in the tropical Pacific
and therefore the correct ENSO amplitude.

The projection corrections decreased monotonically during training ($\delta T$:
$3\times10^{-4}$\,\textdegree C $\to$ $2\times10^{-4}$\,\textdegree C;
$\delta S$: $8\times10^{-5}$\,psu $\to$ $2\times10^{-5}$\,psu), indicating
that the model is learning to self-close the budgets rather than relying on the
post-hoc correction.

\subsection{Remaining challenges and next steps}

\begin{itemize}[itemsep=2pt]
  \item \textbf{Complete run-4 curriculum}: training is currently at step 24\,000
    (rl=32 phase). The rl=64 transition at step 40\,000 is the next milestone;
    reaching rl=64 will expose the model to 64-month compound rollouts and
    is expected to further reduce long-horizon drift.
  \item \textbf{Momentum/barotropic projection}: the conservation projection in
    run~4 closes heat and salt budgets. Extending it to barotropic volume
    conservation (via a Helmholtz-type Poisson solve on the tripolar grid)
    would complete the hard-constraint picture for all conserved quantities.
  ...
  \item \textbf{Spectral regularization}: a small penalty on high-wavenumber
    energy growth may slow residual drift accumulation (in the momentum fields,
    which the projection does not currently correct).
\end{itemize}
